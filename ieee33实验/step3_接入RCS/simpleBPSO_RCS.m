...
function [fy_max] = simpleBPSO_RCS(~,~)
...
for i = 1:N
    all_idx = 1:d;
    idx = all_idx(randperm(length(all_idx), 3));
    x(i,idx) = 1;
end
...
for i = 1:N
    ...
    elseif current_sum < 3
        ...
        [~, order] = sort(v(i, zero_idx), 'descend');  
        % 计算需要补齐的数量
        add_count = 3 - current_sum; 
        % 挑选速度最大的前 add_count 个位置
        add = zero_idx(order(1:add_count));
        % 设为 1
        x(i, add) = 1;
    end
end
...
figure(6);
subplot(1,2,1); bar(y_max); title('simpleBPSO_RCS最终选址结果');
