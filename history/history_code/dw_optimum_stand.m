...
function [t, U, P, Q, P_dr_out, f, P_en, P_loss] = dw_optimum_stand(LC_wt, LC_pv, C_res, r, x, pload, qload,branch,pv,wt)
...
% 计算光伏风电出力
pv = pv/sum(LC_pv/1000);
wt = wt/sum(LC_wt/1000);
...
P_pv = repmat(reshape(P_pv, [33,24,1]), [1,1,25]);
P_wt = repmat(reshape(P_wt, [33,24,1]), [1,1,25]);
size(P_pv)

% 完成节点风电光伏的赋值 由于第一个节点由1变成了33 所以节点编号都要向前移一位
% tic
% for i = 1:33
%     if i == 12 || i == 16 || i == 24
...
% C_res_pv = C_res(:,1:24)';
% C_res_pv = permute(repmat(C_res_pv, [1, 1, 33]), [3, 1, 2]);
% C_res_wt = C_res(:,25:48)';
% C_res_wt = permute(repmat(C_res_wt, [1, 1, 33]), [3, 1, 2]);
% ratio_pv = repmat(reshape(ratio_pv,[33,1,1]), [1,24,25]);
% ratio_wt = repmat(reshape(ratio_wt,[33,1,1]), [1,24,25]);
% pv_3d = repmat(reshape(pv, [1,24,1]), [33,1,25]);
% wt_3d = repmat(reshape(wt, [1,24,1]), [33,1,25]);
% P_pv = C_res_pv.*ratio_pv ;
% P_wt = C_res_wt.*ratio_wt;
toc
...
% 限制条件
    ...
    % cv = [cv;
    % Iij >= 0; % 猜想:U是标幺值的情况下 Iij应该也是标幺值
    % Iij <= 6];%基于熊壮壮程序给电流一个约束
    ...
    P_dr_out >= 0;
    P_dr_out <= u_out*37.15*0.001;
    P_dr_in >= 0;
    P_dr_in <= u_in*37.15*0.001
    ];
