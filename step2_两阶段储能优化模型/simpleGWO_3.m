...
for iter = 1:Max_iter
    %% ---------- 适应度计算 ----------
    tic
    fx = down_capacity(x);   % ⚠️ 核心：与PSO 完全一致
    toc
    %% ---------- 更新 Alpha / Beta / Delta ----------
    for i = 1:N
        if fx(i) < Alpha_score
    ...
    for i = 1:N
        for j = 1:d
            ...
            % --- 更新位置 ---
            x(i,j) = (X1 + X2 + X3) / 3; 
            % 对应论文的区别:x(i,j) = x(i,j) + (X1 + X2 + X3) / 3; -取平均值的物理意义是：普通狼认为猎物最可能在这三个点构成的中心区域,论文这么修改让结果变得更坏了
            % --- 改进：步长平滑 ---
            % 不要直接跳过去，而是以 0.5 的权重平滑移动，减少越限概率
            % X_target = (X1 + X2 + X3) / 3;
            % x(i,j) = 0.5 * x(i,j) + 0.5 * X_target;
        end
    end
    %% ---------- 边界约束 (随机重置法) ----------
    for i = 1:N
