...
function [obj] = evaluate_population(pop_size, LC_wt, LC_pv, P_wt, P_pv, d, P_en, t, P_loss, P_dr, V)  
...
C_l = 24*(0.16 * sum(6200 .* LC_wt(i)) + 0.16 * sum(7000 .* LC_pv(i))); % kw 和 kwh 单位换算
...
for s = 1:N_s
    for i = 1:N_n
        ...
        C_om = C_om + t * 24 * (0.2 * P_wt(s,i) + 0.2 * P_pv(s,i)); % kw 和 kwh 单位换算 调整计算单位 一天24h
    end
end
...
% 2.电压偏差总量
% abs(1 - V) 计算每个节点的电压偏差绝对值
% V表示场景s中节点i在时间t的归一化电压值
F2 = 0;
for s = 1:N_s
    for i = 1:N_n
        for t = 1:24
            F2 = F2 + abs(1 - V(s,i,t)) * t(s);
        end
    end
end
